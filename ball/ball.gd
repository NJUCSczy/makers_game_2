...
func _physics_process(delta):
    ...
    while collision and LengthLeft>0 and (Speed*delta).length()>0: 
        ...
        elif collision.collider.collision_layer & 0b10000 and Status=="hot":
            collision.collider.block_break()
        ...
        var SpeedCost
        if Status=="wet":
            SpeedCost=collision.normal.normalized()*(Speed.length()*cos(angle1-angle2)+collision.collider.CollisionEnergyExtraCost)
        elif collision.collider.name.find("Mail")!=-1 or collision.collider.name.find("Ball")!=-1:
            SpeedCost=Vector2()
...
func change_status(_status):
    Status=_status
    $AnimatedSprite.animation=Status
